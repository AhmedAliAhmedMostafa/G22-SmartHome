...
void ADC0_init()
{
  ...
  ADC0_ACTSS_R&=~ADC_ACTSS_ASEN3;   /* disable SS3 during configuration */
	...
	ADC0_SSMUX0_R =ADC_SSMUX3_MUX0_S; /* get input from channel 0 */
  ...
  GPIO_PORTF_AFSEL_R |= 8; 					/* enable alternate function */
  GPIO_PORTF_DEN_R &= ~8; 					/* disable digital function */
  GPIO_PORTF_AMSEL_R |= 8; 					/* enable analog function */
 }
uint8_t ADC_read(){
